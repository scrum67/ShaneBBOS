...
function shellUser(args) {
    ...
    else _StdIn.putText(String(user));
...
function shellStatus(args) {
    ...
    else _StdIn.putText("Usage: status <update>");
}
...
function shellLoad(args) {
    ...
    if (input.length <= 0) {
        ...
        for (var i = 0; i < commands.length; i++) {
            if (commands[i].length > 2) bool = false;
            else if (commands[i].match(/[0-9a-f]+/i) === "") bool = false;
        }
...
function shellRun(args) {
    if (args.length > 0) {
        ...
        if (bool) {
            _ReadyQueue.push(_ResidentList[parseInt(args)]);
            if(_CurrentProcess === null) {
                _CurrentProcess = _ResidentList[parseInt(args)];
                _Scheduler.updateRQ();
            }

            _CPU.isExecuting = true;
        }
        else _StdIn.putText("Invalid PID specified");
    }
    else _StdIn.putText("Usage: run <pid>");
}
...
function shellCreate(args) {
    ...
    if (kfnFileSysDriver.createFile(filename) === true) _StdIn.putText("File created successfully.");
}
...
function shellWrite(args) {
    ...
    if (kfnFileSysDriver.writeFile(filename, fileData) === true) _StdIn.putText("Write to file completed successfully.");
    else _StdIn.putText("Error in writing to file.");
}
...
function shellDelete(args) {
    var filename = args[0];
    if (kfnFileSysDriver.deleteFile(filename) === true) _StdIn.putText("File deleted successfully.");
    else _StdIn.putText("Error in deleting file.");
}
...
function shellSetSchedule(args) {
    ...
    else _StdIn.putText("Usage: setschedule <fcfs | rr | priority>");
    _Scheduler.updateRQ();
}
